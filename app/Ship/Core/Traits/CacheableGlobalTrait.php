 ...
 * @ Modified time: 2022-08-23 19:10:39
...
trait CacheableGlobalTrait
{
    ...
    public function makeCacheKey($customCacheKey = null, $excludes = [], $debug = 0): string
    {
        ...
        try {
            $key = md5(sprintf("$cacheKey %s %s", $backtrace['class'] . ':' . $backtrace['function'], \serialize($backtrace['args'])));
        } catch (Exception $e) {
            // throw $e;
            ...
            $key = md5(sprintf("$cacheKey %s %s", $backtrace['class'] . ':' . $backtrace['function'], ''));
        }
